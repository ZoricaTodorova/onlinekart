...
$firma_opis=mysqli_fetch_row($firma_cmd);
	...
	$cmd=" select godina,fbr_user, firma, pom as rbr, pom, 
			 ...
			 iznos_v ,	ddv_v, akciza_i, cena_vk as cena_vvk, iznos_vk as iznos_vvk, 
			 iznos_v - kolicina*akciza as iznos, cena,  akciza, 
...
while ($totali=mysqli_fetch_array($totali_cmd)){
	...
	if ($mat=='0'){
		...
		$akc=$akc+($totali['akciza']*$totali['kolicina']);
...
class PDF extends FPDF
{
	...
	function LoadData($file)
	{
		...
		function FancyTable($header,$data)
		{
			...
			foreach($data as $row)
				{ //tuka gi pravime kolonite
					...
					$current_y = $this->GetY();
					$current_x = $this->GetX();
					$this->multiCell($w[0],6,$row['mat']."\n".'   ',1,'C',$fill);
					$this->SetXY($current_x + $w[0], $current_y);
					
					$current_y = $this->GetY();
					$current_x = $this->GetX();
					$this->multiCell($w[1],6,$row['matime']."\n".'Акциза',1,'C',$fill);
					$this->SetXY($current_x + $w[1], $current_y);
					
					$current_y = $this->GetY();
					$current_x = $this->GetX();
					$this->multiCell($w[2],6,$row['edm']."\n".'   ',1,'C',$fill);
					$this->SetXY($current_x + $w[2], $current_y);
					
					$current_y = $this->GetY();
					$current_x = $this->GetX();
					$this->multiCell($w[3],6,number_format($row['kolicina'],2)."\n".'   ',1,'R',$fill);
					$this->SetXY($current_x + $w[3], $current_y);
					
					$current_y = $this->GetY();
					$current_x = $this->GetX();
					$this->multiCell($w[4],6,number_format($row['cena'],2)."\n".number_format($row['akciza'],2),1,'R',$fill);
					$this->SetXY($current_x + $w[4], $current_y);
					
					$current_y = $this->GetY();
					$current_x = $this->GetX();
					$this->multiCell($w[5],6,number_format($row['iznos'],2)."\n".number_format($row['akciza']*$row['kolicina'],2),1,'R',$fill);
					//$this->SetXY($current_x + $w[5], $current_y);
					//$this->Ln();
					$fill=!$fill;
				}
		...
		$pdf=new PDF();
		//Column titles
			$header=array('Шифра на артикл','Назив на артиклот','едм.','Количина','Цена','Износ');
		//Data loading
...
// 			exit();
			...
			if ($netD<>0){
				$pdf->Cell(90);
				$pdf->Cell(40,5,'Нето износ деривати:',1,0,'L');
				$pdf->Cell(25,5,number_format($netD,2),1,1,'R');
			}
			if ($netoD<>0){
				$pdf->Cell(90);
				$pdf->Cell(40,5,'Акциза:',1,0,'L');
				$pdf->Cell(25,5,number_format($akc,2),1,1,'R');
			}
			if ($netoD<>0){
				$pdf->Cell(90);
...
// 			$pdf->Ln(20);
			...
			$pdf->Cell(-190);
			$pdf->Cell(0,10,'Напомени',0,0,'L');
			$pdf->SetFont('Arial','',7);
			$pdf->SetTextColor(0,0,0);
			$pdf->Cell(-190);
			$pdf->Cell(0,20,'Ве молиме најдоцна до наведениот датум за плаќање да го подмирите Вашиот долг. За секое задоцнување',0,0,'L');
			$pdf->Cell(-190);
			$pdf->Cell(0,30,'Ви се пресметува законска казнена камата.',0,0,'L');
			...
			$pdf->Cell(-15);
			$pdf->Cell(20,15,'__________________________________',0,0,'L');
