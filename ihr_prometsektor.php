...
genmenu();
echo "<h1 align=center>Промет по сектори</h1>";
...
elseif(isset($_POST['tot_plakjanja']) && !EMPTY($_POST['tot_plakjanja']) && isset($_POST['tot_datum']) && !EMPTY($_POST['tot_datum'])){
	...
	$sel="select datum, grupa_e, ihrdbrussia.grpsee.dsc as grp_opis, org_e, ihrdbrussia.see.dsc as org_opis, 
				   mdl, wra.mdl.dsc as mdl_opis, sum(iznos) as iznos from promet 
				left join ihrdbrussia.see on promet.org_e=ihrdbrussia.see.cod
				left join ihrdbrussia.grpsee on promet.grupa_e=ihrdbrussia.grpsee.cod
				left join wra.mdl on promet.mdl=wra.mdl.cod
				WHERE datum between $oddat and $dodat $string_org_e $string_grupa_e
				group by grupa_e,org_e,mdl with rollup";
	$rez=mysqli_query($handle, $sel);
}

echo $sel;
$orged_sel=mysqli_query($handle, 'SELECT cod,dsc from see');
$grupa_sel=mysqli_query($handle, 'SELECT cod,dsc from grpsee');
$mdl_sel=mysqli_query($handle, 'SELECT cod,dsc from mdl');
...
while ($row=mysqli_fetch_array($rez)){
	if (empty($row['mdl']) && !empty($row['org_e']) && !empty($row['grupa_e'])){
		echo "<tr>";
		echo "<td  align = 'center' style='height: 70%; width: 20%'><b>" . "Вк за ".$row['org_opis'] ."</b></td>";
		echo "<td  align = 'right' style='height: 70%; width: 20%'><b>" . number_format($row['iznos'],0) . "</b></td>";
		echo "</tr>";
	}
	elseif(empty($row['org_e']) && empty($row['mdl']) && !empty($row['grupa_e'])){
		echo "<tr>";
		echo "<td  align = 'center' style='height: 70%; width: 20%'><b>" . "Вк за ".$row['grp_opis'] ."</b></td>";
		echo "<td  align = 'right' style='height: 70%; width: 20%'><b>" . number_format($row['iznos'],0) . "</b></td>";
		echo "</tr>";
	}elseif(empty($row['grupa_e']) && empty($row['org_e']) && empty($row['mdl'])){
		echo "<tr>";
		echo "<td  align = 'center' style='height: 70%; width: 20%'><b>" . "ТОТАЛ" ."</b></td>";

		echo "<td  align = 'right' style='height: 70%; width: 20%'><b>" . number_format($row['iznos'],0) . "</b></td>";
		echo "</tr>";
	}else{
		echo "<tr>";
		echo "<td  align = 'center' style='height: 70%; width: 20%'>" . $row['mdl_opis'] ."</td>";
		echo "<td  align = 'right' style='height: 70%; width: 20%'>" . number_format($row['iznos'],0) . "</td>";
		echo "</tr>";
	}
