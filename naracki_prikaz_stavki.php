...
if (isset($_GET['glava_id'])){
	...
	$cmd_vknar_nerealizirani=mysqli_query($handle, "select (select cod from firmi where tip='F' and cod='$firma') as firma,
			...
			where nar_glava.firma='$firma' and nar_glava.nalb='' and nar_glava.brisi=0 and nar_stavki.brisi=0 and nar_glava.godina='$god'");
	...
	$cmd_check=mysqli_query($handle, " select if(f_saldo<0 and limit_f,0,1) as dozvola, f_saldo, fin_limit,
			...
			from nar_glava join nar_stavki on nar_glava.id=nar_stavki.nar_glava_id where nar_glava.firma='$firma' and nar_glava.id=$glava_id and nar_glava.brisi=0 and nar_stavki.brisi=0 and nar_glava.godina='$god') as nar
...
<?php 
	...
	$res=mysqli_query($handle, "Select * from nar_stavki where nar_glava_id=".$glava_id." and brisi=0 order by id desc");
	...
	while($row = mysqli_fetch_array($res))
	{
		...
		echo "<tr>";
		if ($propusteno<>0){
			echo "<td align='center' style='color:red'>" . $row['mat']."</td>";
		}else{echo "<td align='center'>" . $row['mat']."</td>";}
		
		if ($propusteno<>0){
			echo "<td align='center' style='color:red'>". $opis_mat[0] . "</td>";
		}else{echo "<td align='center'>". $opis_mat[0] . "</td>";}
		
		if ($propusteno<>0){
			echo "<td align='center' style='color:red'>" . number_format($row['kolicina'],1) . "</td>";
		}else{echo "<td align='center'>" . number_format($row['kolicina'],1) . "</td>";}
		
		if ($propusteno<>0){
			echo "<td align='center' style='color:red'>" . number_format($row['cena'],0) . "</td>";
		}else{echo "<td align='center'>" . number_format($row['cena'],2) . "</td>";}
		
		if ($propusteno<>0){
			echo "<td align='center' style='color:red'>" . number_format($row['rabat'],2) . "</td>";
		}else{echo "<td align='center'>" . number_format($row['rabat'],2) . "</td>";}
		
		if ($propusteno<>0){
			echo "<td align='center' style='color:red'>" . number_format($row['cena_r'],0) . "</td>";
		}else{echo "<td align='center'>" . number_format($row['cena_r'],2) . "</td>";}
		
		if ($propusteno<>0){
			echo "<td align='center' style='color:red'>" . number_format($row['vk_vr'],0) . "</td>";
		}else{echo "<td align='center'>" . number_format($row['vk_vr'],2) . "</td>";}
